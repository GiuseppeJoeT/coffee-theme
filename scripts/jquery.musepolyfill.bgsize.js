    ...
    if (!Muse.Browser.Features.checkCSSFeature("background-size")) {
        var c = function(a) {
            ...
            if (a === document.body)
                ...
                g = b(c),
                h.src = g,
                c.css("background-attachment") == "fixed" ? (i.style.position = "fixed",
                n = !1) : i.style.position = "absolute";
            else if (c.is("#page"))
                ...
                if (c.data("hasBackgroundPositionScrollEffect") != !0)
                    for (var g = 0, k = d.length; g < k; g++)
                        ...
                        case "50%":
                            g == 0 ? (h.style.right = "",
                            h.style.left = "50%",
                            h.style.marginLeft = "-" + Math.ceil(h.offsetWidth / 2) + "px") : (h.style.bottom = "",
                            h.style.top = "50%",
                        ...
                        case "bottom":
                            ...
                            break;
                        default:
                            g == 0 ? (h.style.left = d[g],
                            h.style.marginLeft = "-" + Math.ceil(h.offsetWidth / 2) + "px") : (h.style.top = d[g],
                            h.style.marginTop = "-" + Math.ceil(h.offsetHeight / 2) + "px")
                        }
            }
            ;
            this.resizeImage = function(b) {
                ...
                k < l == b ? (h.style.height = d + 1 + "px",
                h.style.width = "auto") : (h.style.width = g + 1 + "px",
                h.style.height = "auto")
            }
            ;
        ...
        a.prototype.initialize = function(a) {
            ...
            a.each(function() {
                var a = new c(this);
                this !== document.body ? b.updateList.push(a) : (d(window).resize(function() {
                    setTimeout(function() {
                        a.update()
                    }, 10)
                }),
        ...
        if (-1 != a) {
            ...
            for (var a = document.getElementsByTagName("meta"), b = 0, c = a.length; b < c; b++) {
                ...
                if ("generator" == d.getAttribute("name")) {
                    "2017.1.0.379" != d.getAttribute("content") && Muse.assets.outOfDate.push("jquery.musepolyfill.bgsize.js");
                    break
                }
            }
        }
